...
# RR: Copiar standalone.xml
COPY standalone-cfg/ /standalone-cfg/
RUN cp /standalone-cfg/standalone.xml /opt/eap/standalone/configuration/standalone-procesar.xml
RUN chown -R jboss:root /opt/eap/standalone/configuration/standalone-procesar.xml

# RR: Copiar toda la configuración específica de ProceSAR
RUN mkdir /opt/eap/cfg
RUN chown -R jboss:root /opt/eap/cfg/
# COPY <path relativo a Dockerfile>/configuracion_buena/ /opt/eap/cfg/
# RUN chown -R jboss:root <archivos de configuración recién copiados>

# RR: Copiar aplicación de máquina de desarrollo a interior de imagen
COPY bin/ticket-monster.war /opt/eap/standalone/deployments/ROOT.war
RUN chown -R jboss:root /opt/eap/standalone/deployments/ROOT.war

## RR: Verificaciones
RUN ls -la /opt/eap/standalone/configuration
RUN ls -la /opt/eap/cfg
RUN ls -la /deployments
RUN ls -ls /standalone-cfg
...
#ENTRYPOINT "/opt/eap/bin/standalone.sh -c /standalone-cfg/standalone.xml"
CMD ["/opt/eap/bin/standalone.sh", "-b", "0.0.0.0", "-c", "standalone-procesar.xml"]
